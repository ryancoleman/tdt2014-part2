...
#declare this here
radiiDefault = {'C':1.9,'O':1.6,'N':1.65,'P':1.9,'S':1.9,'H':0.,'F':0.,'I':0., \
         'U':0.,'A':0.,'B':0.,'L':0.,'*':0.,'Z':0.,'D':0.,'K':0.,'M':0.} 
...
class pdbData(object):
  ...
  def __init__(self, filename=False, hetOnly=False, radiiOverride=None):
  ...
  def setRadiiToFactors(self):
    ...
    for rawDataIndex in xrange(len(self.rawData)):
      newFactors = (self.radii[rawDataIndex], \
          self.factors[rawDataIndex][occPlace])
      self.updateFactors(rawDataIndex, newFactors)
  ...
  def setChargeToFactors(self, chargeZero=False):
    ...
    for rawDataIndex in xrange(len(self.rawData)):
      if not chargeZero:
        newFactors = (self.factors[rawDataIndex][occPlace], \
            self.charges[rawDataIndex])
      else:
  ...
  def getListResiduesChains(self, residueChainNumbers):
    ...
    for resChainNum in residueChainNumbers:
      resChainCheck.append(resChainNum + '+') #suffix of + required for matching
    newPdb = self.copy()
    markedForRemoval = []
    for index, thisResNum in enumerate(newPdb.resNums):
  ...
  def renameHistidines(self):
    ...
    for rawDataIndex in xrange(len(self.rawData)):
      if self.resNames[rawDataIndex] == "HIS": #only care about unchanged histidines
  ...
  def selectMostOccupied(self, exceptions=[], leaveAlone=[]):
    ...
    sameAtoms = collections.defaultdict(list) #collect lists of atoms to examine
    for atomCount in xrange(len(self.atoms)):
  ...
  def alignOntoThis(self, otherPdb):
    ...
    import kabsch #only needed here for now, needs numpy so don't want to always
    import numpy
    ...
    for chainResNum, indexList in otherResSets.iteritems():
      ...
      try:
        #print otherList[:4], xyzList[:4], otherList
        newCoords = kabsch.kabsch_align_other_notnumpy(otherList[:4],\
            xyzList[:4], otherList)
